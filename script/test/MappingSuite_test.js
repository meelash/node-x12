...
(0, mod_js_1.describe)("X12Mapping", () => {
    ...
    (0, mod_js_1.it)('should return null for nonexistent values when using FOREACH', () => {
        const parser = new mod_js_2.X12Parser();
        const interchange = parser.parse(edi);
        const transaction = interchange.functionalGroups[0].transactions[0];
        const mapper1 = new mod_js_2.X12TransactionMap({ TestKey: 'PR03' }, transaction);
        const wOutFOREACH = mapper1.toObject();
        const mapper2 = new mod_js_2.X12TransactionMap({ TestFOREACH: 'FOREACH(PO1)=>PO102', TestKey: 'PR03' }, transaction);
        const wFOREACHFirst = mapper2.toObject();
        const mapper3 = new mod_js_2.X12TransactionMap({ TestKey: 'PR03', TestFOREACH: 'FOREACH(PO1)=>PO102' }, transaction);
    ...
    (0, mod_js_1.it)('should resolve array and nested map values after a FOREACH', () => {
        const parser = new mod_js_2.X12Parser();
        const interchange = parser.parse(edi);
        const transaction = interchange.functionalGroups[0].transactions[0];
        ...
        const rows = new mod_js_2.X12TransactionMap(map, transaction).toObject();
    ...
    (0, mod_js_1.it)('should not share object values between FOREACH rows', () => {
        const parser = new mod_js_2.X12Parser();
        const interchange = parser.parse(edi);
        const transaction = interchange.functionalGroups[0].transactions[0];
